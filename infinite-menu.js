...
function createProgram(gl, shaderSources) {
    ...
    gl.bindAttribLocation(program, 1, 'aModelNormal');
...
class ArcballControl {
    ...
    update(deltaTime, targetFrameDuration = 16) {
        ...
        const snapRotation = glMatrix.quat.create();
        ...
        if (this.isPointerDown) {
            ...
            if (glMatrix.vec2.squaredLength(midPointerPos) > this.EPSILON) {
...
class InfiniteGridMenu {
    constructor(canvas, items, onActiveItemChange, onMovementChange, scale = 1.0) {
        this.canvas = canvas;
        this.items = items;
        ...
        this.camera = {
            ...
            fov: Math.PI / 4,
            aspect: 1,
            position: glMatrix.vec3.fromValues(0, 0, 3 * scale),
    ...
    initDiscInstances(count) {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            glMatrix.mat4.identity(instanceMatrixArray);
    ...
    onControlUpdate(deltaTime) {
        ...
        if (!this.control.isPointerDown) {
            ...
            const snapDirection = glMatrix.vec3.normalize(glMatrix.vec3.create(), this.getVertexWorldPosition(nearestVertexIndex));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Convert photo config to items format
    const items = photoConfig.photos.map(photo => ({
        image: photo.src,
        link: '#',
    ...
    const handleActiveItem = (index) => {
        currentActiveItem = items[index];
        if (titleEl && descEl) {
            titleEl.textContent = currentActiveItem.title;
            descEl.textContent = currentActiveItem.description;
        }
    };
